...
import {
  ...
  type ProductTruthLegacyBridgePreflightReport,
...
type ExactApplyOptions = ApplyBaseOptions & {
...
type StandingApplyOptions = ApplyBaseOptions & {
  authorizationMode: "standing";
  standingPolicyPath: string;
  expectedStandingPolicySha256: string;
  preflightReportPath: string;
  expectedPreflightReportSha256: string;
};

type ApplyOptions = ExactApplyOptions | StandingApplyOptions;
...
function usage(): string {
  return [
    "Usage:",
    "  npm run product-truth:legacy-bridge-wave -- plan",
    "    --snapshot ABS_PATH --bridge-plan ABS_PATH",
    "    --listing-key KEY (1-50 explicit unique listings)",
    "    --created-at ISO --expires-at ISO --out ABS_NEW_DIR",
    "",
    "  npm run product-truth:legacy-bridge-wave -- preflight",
    ...
    "",
    "  npm run product-truth:legacy-bridge-wave -- apply",
    "    (--url URL | --url-env ENV_NAME) [--allow-remote --auth-token-env ENV]",
    "    --snapshot ABS_PATH --bridge-plan ABS_PATH",
    "    --apply-plan ABS_PATH --plan-sha256 SHA256",
    "    (--approval ABS_PATH --approval-sha256 SHA256 --confirmation EXACT_TOKEN",
    "     | --standing-policy ABS_PATH --standing-policy-sha256 SHA256",
    "       --preflight-report ABS_PATH --preflight-report-sha256 SHA256)",
    "    --started-at ISO --out ABS_NEW_DIR",
    "",
    "Safety:",
    "  plan is offline/read-only.",
    "  apply writes only the exact owner-approved graph-aware wave in one transaction.",
...
function parseOptions(argv: readonly string[]): CliOptions {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    const allowed = new Set([
      ...
      "--confirmation", "--standing-policy", "--standing-policy-sha256",
      "--preflight-report", "--preflight-report-sha256",
      "--started-at", "--checked-at",
    ]);
  ...
  const exactAuthorizationFlags = [
    "--approval",
    "--approval-sha256",
    "--confirmation",
  ].filter((flag) => values.has(flag));
  const standingAuthorizationFlags = [
    "--standing-policy",
  ...
  if (
    (exactAuthorizationFlags.length !== 3 || standingAuthorizationFlags.length !== 0)
    && (standingAuthorizationFlags.length !== 4 || exactAuthorizationFlags.length !== 0)
  ) {
    fail(
      "AUTHORIZATION_FLAGS_INVALID",
      "provide one complete exact-owner or standing-policy authorization set",
    );
  }
  const applyBase = {
    command,
  ...
  if (exactAuthorizationFlags.length === 3) {
    return {
      ...applyBase,
      authorizationMode: "exact",
  ...
  return {
    ...
    expectedStandingPolicySha256: exactSha(
      required("--standing-policy-sha256"),
      "--standing-policy-sha256",
    ),
    preflightReportPath: exactPath(
      required("--preflight-report"),
      "--preflight-report",
    ),
    expectedPreflightReportSha256: exactSha(
      required("--preflight-report-sha256"),
      "--preflight-report-sha256",
    ),
  };
}
...
async function runPlan(options: PlanOptions): Promise<void> {
  ...
  const index = {
    schemaVersion: "product-truth-legacy-bridge-wave-artifact-index/2.0.0",
...
async function runApply(options: ApplyOptions): Promise<void> {
  ...
  try {
    ...
    let report;
    if (options.authorizationMode === "exact") {
      const approvalJson = await readExact(options.approvalPath);
      const approval = JSON.parse(approvalJson) as ProductTruthLegacyBridgeApproval;
      report = await applyProductTruthLegacyBridgeWave({
        ...commonApply,
        approval,
        approvalJson,
      ...
      const [standingPolicyJson, preflightReportJson] = await Promise.all([
        readExact(options.standingPolicyPath),
        readExact(options.preflightReportPath),
      ]);
      report = await applyProductTruthLegacyBridgeWave({
        ...commonApply,
        standingPolicy: JSON.parse(
          standingPolicyJson,
        ) as ProductTruthLegacyBridgeStandingPolicy,
        standingPolicyJson,
        standingPolicySha256: options.expectedStandingPolicySha256,
        standingPreflightReport: JSON.parse(
          preflightReportJson,
        ) as ProductTruthLegacyBridgePreflightReport,
        standingPreflightReportJson: preflightReportJson,
        standingPreflightReportSha256: options.expectedPreflightReportSha256,
      });
    }
    ...
    const indexJson = renderProductTruthOperationalJson({
      schemaVersion: "product-truth-legacy-bridge-wave-result-index/2.0.0",
...
async function runPreflight(options: PreflightOptions): Promise<void> {
  ...
  try {
    const report = await preflightProductTruthLegacyBridgeWave({
      db,
      databaseTargetFingerprint: target.fingerprint,
      plan,
      planJson,
    ...
    const reportSha256 = productTruthLegacyBridgeBytesSha256(reportJson);
    const indexJson = renderProductTruthOperationalJson({
      schemaVersion: "product-truth-legacy-bridge-wave-preflight-index/2.0.0",
