...
IF carrierDelayDetected = true AND carrierBadge = "Claims Protected" AND shippedOnTime = true:
  ...
  → Internal note: "НЕ делать прямой refund — ветка A, Claims Protected badge. Ждать A-to-Z от клиента."

IF carrierDelayDetected = true AND (carrierBadge = "Late Delivery Risk" OR carrierBadge = "Unknown"):
  → action = "REPLACEMENT" или "REFUND"
  → Наша ответственность — предложить замену или возврат
  → Internal note: "Нет защиты Claims Protected. Наша ответственность."
...
IF carrierDelayDetected = false OR не видно данных на скриншоте:
  ...
  → Internal note: "Проверить трекинг в Veeqo/Control Center — не видно на скриншоте"
```

### A-to-Z Guarantee — Пошаговая инструкция для клиента (English):

```
To file your A-to-Z Guarantee claim:
1. Go to amazon.com/your-orders
2. Find order [Order ID]
3. Click "Problem with order"
4. Select "Package didn't arrive" or "Item arrived damaged/defective"
5. Follow the prompts to submit your claim
6. Amazon will review and issue a refund within 1-2 weeks
```

### A-to-Z Guarantee — Пошаговая инструкция для клиента (Spanish):

```
Para presentar su reclamo de Garantía A-to-Z:
1. Vaya a amazon.com/your-orders
2. Busque el pedido [Order ID]
3. Haga clic en "Problema con el pedido"
4. Seleccione "El paquete no llegó" o "El artículo llegó dañado/defectuoso"
5. Siga las instrucciones para enviar su reclamo
6. Amazon revisará y emitirá un reembolso en 1-2 semanas
```
...
*Версия: v1.2 — 2026-04-07*
*Для: Salutem Solutions Control Center*
*Модуль: Customer Service*

*Изменения v1.1 → v1.2:*
*- Добавлен раздел "Carrier Delay Detection" с полным алгоритмом определения задержки перевозчика*
*- Добавлены новые actions: A2Z_GUARANTEE, PHOTO_REQUEST*
*- Добавлены поля carrier delay: carrierDelayDetected, carrierBadge, shippedOnTime, promisedEdd, actualDelivery, daysLate*
*- Добавлена пошаговая инструкция A-to-Z Guarantee для клиентов (English + Spanish)*
*- Добавлен новый статус кейса "responded" (open → responded → resolved)*
*- Уточнено правило: НИКОГДА не предлагать прямой refund при Claims Protected + carrier delay*
