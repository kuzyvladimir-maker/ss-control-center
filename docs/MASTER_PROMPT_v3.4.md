...
|------------|---------|----------|
| `critical` | **2** дня | v3.4 override — экстремальная жара / multi-day high-temp в маршруте → еда под угрозой даже при on-time доставке. Алгоритм отсекает любой Ground / Saver вариант с EDD > 2 дней. |
| `high`/`medium`/`low`/`ok`/нет alert | **3** дня | Дефолт v3.3 — обычный food-safety запас. |
...
При `risk=critical` (N=2) сдвиг на одну позицию:
- 1 день: идеально
- 2 дня: допустимо
...
> **Имплементация:** `frozenMaxCalDays(riskLevel)` в
