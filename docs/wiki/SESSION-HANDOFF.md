...
> (без PUT-плясок), buy/route.ts матчит сервис регистронезависимо (verified: все 16 сервисов нового API
> матчатся со старым пулом → покупка не откажет). tsc чисто. **Пуш сделан — Владимиру протестировать на
> /shipping: перезагрузить, проверить frozen-карточку + 1 тестовая покупка перед массой.**
