...
>
> **Последнее обновление:** 2026-06-12 (Claude — спланирована **Walmart Quantity-Confusion Fix**: диагноз
...
>
> **2026-06-12 (MacBook-Claude, вечер) — ОТДЕЛЬНЫЙ ТРЕД: Shipping Labels.** Куча правок зашипано (см. блок
> «🚢 SHIPPING» ниже). **КРИТИЧЕСКОЕ незакрытое:** Frozen Monday-shift / пересчёт рейта по дате отгрузки
> **сломан в корне** — публичный Veeqo API не даёт котировать отгрузку позже дедлайна Amazon. Карточка
> сейчас может ВРАТЬ (показывает понедельничный рейт по сегодняшним EDD → риск купить рейт, который привезёт
> размороженным). Владимир настаивает, что это работало пару дней назад. **Решение НЕ принято.** Детали ↓.
...
## 🆕 СЕССИЯ 2026-06-12 (Quantity-Confusion Fix) — ПЛАН утверждён, кода НЕТ
