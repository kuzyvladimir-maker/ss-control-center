...
>
> **Последнее обновление:** 2026-07-04 (Claude Code, Opus 4.8, iMac ночь) — **✅ COGS-движок «ПОД КЛЮЧ» + 🔧 ПОЧИНЕН ПРОД-БИЛД (не деплоился 24ч+).** COGS: (1) **ручная ступень своих брендов** (Starfit скакалки $0.80 / 2-пак $1.50, Salutem Vita детокс $5.50, Lion's Mane $6.50, Nicotinamide $7.50) — верх лестницы, ноль платных вызовов; (2) **SkuComponent** = структурная спецификация каждого SKU (bundle→компоненты, у каждого цена+метод+ссылка на донора-контент) для экономики + новых/правки листингов; (3) **общий движок `cogs-engine.ts`** (CLI + крон = одна логика); (4) **фоновый крон `/api/cron/cogs-sweep`** (каждые 2ч × 25 SKU ≈ 300/день → ~13 дн на 2 899 PUBLISHED Walmart, потом Amazon; резюмируемый); (5) **UI `/cogs`** «SKU Cost Catalog» (покрытие + кликабельные KPI + спецификация по клику). Лестница цены: own-brand → 1P exact → line-price → Google Shopping (ни один SKU без числа). **⛔ БЛОКЕР: vision-ключ (Anthropic/OpenAI) БЕЗ КРЕДИТОВ** → identify новых SKU падает; **владельцу пополнить API-кредиты**, иначе полный авто-прогон стоит (кэш + own-brand + retail работают: скакалка $0.80, nicotinamide+lion's mane $14.00, Kellogg's $19.92; покрытие 96/2 899). **🔧 Билд-фикс:** корневые `_*.ts` скретч-файлы (`_gen.ts` c `import("./src/…ts")`) ломали `next build` type-check → **весь прод не деплоился сутки+** (включая вчерашний ChannelMAX-фикс) → исключил `_*.ts`/`_*.mjs` в tsconfig, tsc = 0 ошибок. **НЕ коммить `_`-скретчи в корень.** Коммиты `65da994`,`5524347`,`e27c8cf`,`3751752`.
>
