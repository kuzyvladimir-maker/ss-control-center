...
1. **[Jackie, в работе]** Прогнать сервисы по **brain-walmart-batch.json** (запросы мозга, не сырые тайтлы),
   фильтр first-party (is_marketplace=false, pack_size=1), вернуть полный пул инфо → `results/brain-walmart-results.json`.
   Это двойной тест: (а) платный структурный источник vs free, (б) мозг-резолв vs сырой тайтл (9/13).
