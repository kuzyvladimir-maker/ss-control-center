...
3. **Ручной выбор тарифа** (клик по Carrier) → `WalmartPickRateDialog`: показывает тарифы **Walmart** + поле **ship date** (смена даты пере-котирует) + подсветку **on time / misses deadline** на каждом тарифе. Выбор обновляет и показ, и то, что купится, и ship-date заказа.
   - **Дата отгрузки**: глобальное поле **Ship date** вверху страницы (по умолчанию сегодня ET) пере-котирует все Walmart-заказы; дата редактируется и в шапке каждой строки (override подсвечен синим). См. [shipping-labels-page-v1.md](shipping-labels-page-v1.md) → «Дата отгрузки».
...
### Алгоритм выбора тарифа (Walmart)
`selectBestWalmartRate` = **самый дешёвый тариф, который укладывается в срок доставки** (а НЕ просто самый дешёвый): сначала фильтрует тарифы, у которых `deliveryPromiseFulfilled = true` (Walmart сам считает попадание в свой deliver-by), затем берёт минимальную цену среди них. Если ни один не попадает в срок — берёт самый дешёвый из всех (оператор увидит подсветку «misses deadline» и сам решит). Это **тот же Dry-goods rule, что в master-промте** (cheapest-that-meets-deadline). Frozen на Walmart запрещён, поэтому frozen-бэндинг (как в Veeqo-плане) здесь не нужен.
