...
Статус 2026-07-25: **9/12 опубликованы, 8 BUYABLE** (UF wheat-duo добит на
7-й попытке прицельным пунктом про амперсанд; UA variety-54 — на 8-й после
пункта про розовый Berry Burst). Финальная тройка (choc-berry-60, xl-96,
xl-protein-90) НА ПАУЗЕ: составы перекроены под лимиты рендерера (слово
«Mixed»/Beamin' нерендерабельно на мелком масштабе → заменён на
Bright-Eyed/Up & Apple; xl-96 пересобран в форму 2/2/4/3 из доказанных
суб-блоков — структуры теперь держатся стабильно), но после ~60 рендеров за
двое суток image_gen воркера деградировал: 10+ подряд роллов с буквенным
салатом (Batter/8MUCKER'S/Bloeberry-класс) при идеальных каунтах.
Возобновлять роллы в свежем окне квоты (исторически — раннее утро ET);
процедура: рендер → chmod 444 → полный агент 8-16x (мой прескрин 2-3x и
Haiku-фильтр ложнопозитивят — НЕ доверять).
