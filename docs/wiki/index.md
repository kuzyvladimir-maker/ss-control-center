...
- **[Операционный контракт агентов — 2026-07-29](agent-operating-contract-2026-07-29.md) — разбор трёх недель работы и причин, по которым задачи не доходили до «готово»: 231 прозаический SHA, `release-artifacts/` вне git, TTL короче человеческой подписи, 44 % коммитов на координацию, 150 ГБ брошенных чекаутов. Содержит новые правила автономии, параллельной работы Claude ↔ Codex (`scripts/agent-sync.mjs`) и машинных пинов (`scripts/release-pins.mjs`). Читать первым.**
...
- **[Product Truth Web Operations Control Plane](product-truth-web-operations-control-plane.md) — default-OFF web→worker design: отдельный Ed25519 owner domain, immutable command artifacts, typed CLI allowlist, leases/ambiguity и staged activation без права на production/spend.**
- **[Product Truth Owner Gates](product-truth-owner-gates.md) — единый живой decision ledger: независимые G1–G8, точные разрешённые/запрещённые действия и consumed evidence без конкурирующих prompts.**
- **[Product Truth Standing Authority](product-truth-standing-authority.md) — постоянная owner policy для автономного retailer/provider enrichment: `balance-probe → authorize → execute` без ручных approval-фраз, с pinned plan/budget/reserve/safety gates.**
...
- **[Walmart New SKU — item price vs customer shipping](walmart-new-sku-shipping-price-strategy.md) — evidence-based channel policy для seller-fulfilled SKU: landed price, Buy Box/Listing Quality, buyer psychology, формула template split и controlled experiment; Product Truth не меняет.**
...
- [Walmart Maruchan FaisalX-1433–1435 — exact repair contract](walmart-maruchan-faisalx-1433-1435-repair.md) — ordinary Chicken identity correction: profitable prices and Chicken MAIN images succeeded; Product Truth current recipes are exact Chicken with unchanged COGS; a correct Chicken-only text canary proves the remaining title/bullets/attributes are blocked by Walmart unified-catalog content ownership.
...
- [HANDOFF → Codex: batch-200 и что дальше](HANDOFF-uncrustables-batch200-2026-08-13.md) — **передача работы 2026-08-13**: 200 листингов созданы и опубликованы; вся аналитика (промо-эксперимент купон vs sale = ничья; 13 из 163 листингов дают весь трафик; одновкусовые побеждают миксы; индивидуальные упаковки обходят коробки; ОДИН листинг от 10.12.2024 зарабатывает больше всех 335 вместе); что осталось (фильтровые атрибуты + чистые ключевики на 200, новая партия одновкусовых); грабли, на которых терялось время.
- [Uncrustables — КАНОН создания листингов](uncrustables-listing-rules-canon.md) — **единая точка входа перед любой работой с листингами Uncrustables (2026-08-11)**: картинки только композитом из РЕАЛЬНЫХ фото коробок (никакого выдуманного товара — правило владельца 2026-07-08, подтверждено 2026-08-10), количества только от 24шт в рациональных зонах кулеров, цена по markup ≈2.35 от бестселлеров с доставкой отдельной строкой, ROI 70% на вложенные (товар+упаковка). Плюс открытое расхождение: гейт подлинности требует сцену с кулером и не пропускает композит — нужно решение владельца.
...
- [Uncrustables trial run — 12 ASIN через box-planner](uncrustables-trial-run-12.md) — второй боевой прогон конвейера (2026-07-23/24): 8+ листингов ACCEPTED c рецептами и копией из box-planner-модуля, двухслойная верификация (кропы + панель 3 слепых агентов на рендер), контракт рендера дорос до 10 боевых пунктов, sealed-манифест растёт инкрементально (dual-manifest preflight); наблюдение — типографика image-воркера деградирует волнами, лечится ре-роллами.
...
- [Ценообразование studio: ROI на вложенные](walmart-studio-pricing-roi.md) — правило владельца 70% ROI (товар+упаковка, доставка НЕ в знаменателе); чем это отличается от маржи и почему 30% маржи давали 42% ROI при бесплатной доставке
