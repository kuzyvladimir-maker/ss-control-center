...
## 8. ✅ Первый товар опубликован (2026-08-04)

`FN-WM30-XEHS` — **первый товар, созданный в Walmart через наш API**.

| | |
|---|---|
| Фид | `18C8AC793CC8524BA16DD51FB0502609@AXkBBgA` → `PROCESSED`, `ok=1 fail=0` |
| Статус в каталоге | **`PUBLISHED` / `ACTIVE`** |
| Walmart ID | `wpid 3RUAU6HWUWJ9` · `itemid 20765921612` |
| Полка | Food → Pantry → Soup → Ready to heat soup |
| Цена / остаток | $37.68 · 50 |

### Сроки обработки — замерено на трёх заходах

| Заход | Длительность | Исход |
|---|---|---|
| 1 | 63 мин | DATA_ERROR (картинка на r2.dev) |
| 2 | 83 мин | DATA_ERROR (401 на .jpg) |
| 3 | **1 мин** | SUCCESS |

Третий заход быстрый, потому что карточка уже существовала — Walmart дописывал в
неё картинки. **Создание с нуля — около часа.** Подтверждение фида (~2 сек) это
НЕ создание товара.

### Последний дефект: `martId` равен нулю

Опрос статусов не признал успех и выдал противоречие:
`WALMART_FEED_TERMINAL_NOT_SUCCESS: "Exact feed item status is SUCCESS."`

Условие требовало `item.martId`, а Walmart присылает в нём **число 0** —
проверка на истинность отбрасывала успешный товар. Настоящий идентификатор
созданного товара — **`wpid`** (строка), рядом лежит числовой `itemid`.

Поля позиции в `itemIngestionStatus`:
`martId, sku, wpid, index, itemid, productIdentifiers, ingestionStatus`.

Теперь идентификатор резолвится как `wpid → itemid → martId`, причём `"0"` не
считается значением.

### Общий вывод по трём дефектам чтения ответов Walmart

Все три — одного класса: **код читал имена полей, которых Walmart не присылает**.

1. `itemDetails.itemDetails` вместо `itemDetails.itemIngestionStatus`;
2. `martId` вместо `wpid`;
3. `countryOfOriginSubstantialTransformation` вместо snake_case.

Ни один не всплывал раньше, потому что до 2026-08-03 мы **ни разу не создавали
товар** — эти ветки никогда не выполнялись против настоящего ответа.

**Правило:** структуру ответа маркетплейса берём из живого ответа
(`scripts/_feed_raw.ts <feedId>` печатает поля позиции), а не из типа в коде.
