...
### ⚠️ БЛОКЕР найден в 82-прогоне с атрибутами (ночь 2026-07-01)
- Прогон: 79 submitted / 0 fail, все с атрибутами. НО **Anthropic API кончились кредиты в середине прогона** ("credit balance is too low") → polish (генерация текста) упал для последних ~13 SKU → они получили bare-fallback текст. Итог: **66/79 полный текст, 13 bare** (нужен ре-ран после пополнения кредитов). Фото: 41/79 новое главное, 38 нужен ручной рычаг генерации. Галерея≥2: 73/79.
- **Кредиты Anthropic ломают генерацию текста ВЕЗДЕ в приложении** (Customer Hub, bundle factory, remediation) — топ-приоритет пополнить.
- **Пробел в самопроверке:** canary проверяет только ПЕРВЫЕ N. Деградация в СЕРЕДИНЕ (кредиты кончились, API-outage) не ловится. Улучшение на будущее: rolling health-check (если последние K билдов подряд bare → abort). См. [[feedback_self_verify_long_runs]].
- Приёмка атрибутов Walmart: feed-ы обрабатывались очень медленно (>15 мин не terminal), подтверждение отложено; per-item тест ранее уже доказал что SAFE-набор проходит.

### QC-находка владельца 2026-07-06 — Maruchan Pork 8-pack: плитка из КЕЙСОВ (FaisalX-3344)

- Владелец нашёл на живом сайте (item 10990870073): главное фото = **8 открытых кейсов по ~24 пачки** (визуально «192 шт»), а листинг продаёт 8 одиночных пачек. Классический multi-unit-source брак.
- Причина: SKU чинился **2026-06-18 старым движком** (до ужесточения отбора 2026-07-01 «ОДНА единица на белом фоне»). Донором взяли фото кейса и растиражировали 8×.
- Фикс: пере-запуск `buildAndSubmitOne` (scope image-only, `forceImage`, buildOnly) → новый строгий пикер выбрал одиночную пачку → плитка 3-3-2, 2000², verify PASS, проверено глазами → `submitMainImageOnly` feed `18BFC068067E592D9B89A2A99C22FEC7@AX8BBgA`. Запись в WalmartListingRemediation (changeType=`multipack-image-redo`).
- Новая картинка: `walmart-multipack/FaisalX-3344/main-20260706-56223.png` (R2).
- Соседний вариант «Count 6 / $10.48» на той же странице — ЧУЖОЙ листинг (не наш). Наш родственный FaisalX-3345 (Pork, Pack of 10) не ремедиировался вообще — не трогали.
- ⚠️ **Масштаб:** в базе **557 SKU**, у которых последний успешный image-фикс сделан ДО 2026-07-01 (старый движок, без правила «одна единица» и без verify плитки). Среди них наверняка есть ещё такие же «кейсы вместо пачек». Дешёвый способ найти: прогнать `verifyMainImage(mainImageUrl, packCount)` по этим 557 старым плиткам (~1 vision-вызов на SKU) и собрать список на re-do. НЕ запускал — ждёт решения владельца (он и так планирует полный прогон 1857).

### QC-находка владельца 2026-07-16 — Pepperidge Farm Very Thin 6-pack: wrong-product main + разрозненная галерея (FaisalX-1144)

- Владелец увидел на витрине (item 8394316918): главная = плитка **Farmhouse Butter Bread** (не тот продукт), галерея — мешанина чужих фото. Источник брака: волна fresh-50 от 2026-07-01 (`main-20260701-f50.png`), та самая «94% ложные».
- Сиблинги Pack of 2 / Pack of 4 (FaisalX-1142/1143) уже перечинены каноном 07-08/07-13 (`main-enriched.png`, APPLIED). Pack of 6 канон уже пересобрал в фоне (`_pipeline_state.json`: REBUILT_OK, `main-pipeline.png`, донор Walmart 1P, 3×2), но опубликовать не успел.
- Фикс (MacBook, по прямой просьбе владельца, вне батча): опубликован готовый `main-pipeline.png` + **галерея 7 фото** из DonorProduct `5d4f8fb8` (точный продукт, все проверены глазами: фронт/нутриция/состав/оборот/ракурсы). Один MP_MAINTENANCE feed `18C2C9A7AE1D58BDA86FFA4CA8F4AE86@AX8BBgA`, запись `multipack-image-redo` в WalmartListingRemediation. Плашка «WHOLE WHEAT 16oz» у донора — тот же принятый вид, что уже живёт на Pack of 4.
- ⚠️ Для батч-паблишера full-run: FaisalX-1144 уже опубликован досрочно — повторная публикация того же URL безвредна, но можно скипнуть.
