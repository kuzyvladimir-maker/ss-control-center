...
### Amazon — плавающая дата = только «N дней в пути» (2026-05-30, выверено вживую)

**Важный факт, проверенный прямым тестом против Veeqo API:** `getShippingRates(allocation)` отдаёт **фиксированную котировку** и **НЕ пересчитывает её по дате отгрузки**. Тест (`_diag-veeqo-rates-date.ts`, потом удалён): PUT dispatch_date на сегодня / +7 / +14 дней, пауза до 5.5с, сброс кэша аллокации — тарифы И EDD **байт-в-байт одинаковые** на всех датах (EDD `06-04` даже при отгрузке `06-13`, т.е. доставка раньше отправки). Значит цена/перевозчик/EDD от даты отгрузки в Veeqo не зависят.

Поэтому для Amazon **ре-квот не делаем** (он возвращал бы то же самое и зря писал бы в Veeqo). Дата на Amazon-строке редактируема, но меняет **только «N days»** = `EDD − дата отгрузки` (это единственное, что реально зависит от даты — ровно как колонка «2 days» в UI Veeqo, которая тоже просто `EDD − ship date`, а не ре-квот). Цена/EDD/попадание-в-дедлайн не меняются (EDD фиксирован). **Покупка Amazon — на ship-by** (правило владельца).

Это согласуется с Frozen Ship Date Trick (`plan/route.ts`): он PUT-ит понедельник и пере-фетчит, но получает тот же фиксированный EDD; «выигрыш» — в пересчёте `calDays = EDD − понедельник` (меньше дней до фиксированной даты доставки), а не в новой котировке. Trick трогать не стал.

Для **Walmart** ре-квот реальный: SWW estimate принимает `shipByDate` и возвращает другие тарифы/EDD — поэтому там дата drives и показ, и покупку (`quoteWalmartOrder`). Реализация общего: `effectiveShipDate`, `shipDateGlobal`/`shipDateGlobalRef`. SWW `createLabel` поля даты не принимает — лейбл датируется моментом покупки; дата используется для папки Drive и выбора тарифа.
