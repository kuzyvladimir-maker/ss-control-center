...
/**
 * Timesheet (табель) — mark the days each salary employee worked; pay = worked
 * days × per-day rate (weekly/monthly rates are converted to per-day). "Create
 * salary bills" turns each employee's pay into a bill in the Salaries fund, where
 * you mark it paid (debits the fund).
 */

import { useCallback, useEffect, useState } from "react";
import Link from "next/link";
import { Loader2, ArrowLeft, AlertCircle, ChevronLeft, ChevronRight, ReceiptText, CheckCircle2 } from "lucide-react";
...
export default function TimesheetPage() {
  ...
  const [worked, setWorked] = useState<Record<string, Set<string>>>({}); // empId → set of dates
  ...
  async function createBills() {
    ...
    try {
      ...
      setNote(`Created ${r.created} salary bill(s) in the Salaries fund. ${parts}`);
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Mark worked days; salary = days × per-day rate. Create bills → they appear in the Salaries fund to pay.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/finance/expenses"><Button variant="outline" size="sm">Edit rates</Button></Link>
      ...
      {note && <Card className="border-emerald-500"><CardContent className="flex items-center gap-2 py-3 text-sm text-emerald-600"><CheckCircle2 className="h-4 w-4" />{note}</CardContent></Card>}
