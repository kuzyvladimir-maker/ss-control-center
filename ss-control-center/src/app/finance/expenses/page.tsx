...
/**
 * Recurring expenses (OpEx) — the editable master list. Categories ARE the FP1
 * funds (Salaries / Warehouse & Logistics / Software / Subscriptions). Shows the
 * monthly obligation per fund (weekly × 52/12) so you know how much each fund must
 * accumulate. Seeded from Sellerboard; edit amounts as they change.
 */
...
export default function ExpensesPage() {
  ...
  async function patch(id: string, data: Partial<Expense>) {
    await fetch("/api/finance/expenses", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id, ...data }) });
    await load();
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Recurring expenses (OpEx)</h1>
          <p className="text-sm text-muted-foreground">Categories are the FP1 funds. Monthly obligation = how much each fund must cover. Edit amounts as they change.</p>
        </div>
      ...
      {showImport && (
        <Card><CardHeader><CardTitle className="text-base">Import Sellerboard expenses CSV</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            <textarea value={csv} onChange={(e) => setCsv(e.target.value)} rows={6} className="w-full rounded-md border bg-background p-2 font-mono text-xs" placeholder="Paste the CSV (Date;Type;Name;Category;Product;Marketplace;Sum;Currency;Ad_spend)…" />
            <Button onClick={importCsv} disabled={busy || !csv.trim()}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Import"}</Button>
          </CardContent>
        </Card>
      )}

      {/* Monthly obligation per fund */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {byCategory.map((c) => (
          <Card key={c.category}><CardContent className="py-4">
            <div className="text-xs uppercase text-muted-foreground">{c.category}</div>
            <div className="text-xl font-semibold">{usd(c.monthly)}<span className="text-xs text-muted-foreground">/mo</span></div>
          </CardContent></Card>
        ))}
        <Card className="border-primary"><CardContent className="py-4">
          <div className="text-xs uppercase text-muted-foreground">Total OpEx</div>
          <div className="text-xl font-semibold">{usd(monthlyTotal)}<span className="text-xs text-muted-foreground">/mo</span></div>
        </CardContent></Card>
      </div>

      {/* Add expense */}
      <Card><CardHeader><CardTitle className="text-base">Add expense</CardTitle></CardHeader>
        <CardContent className="flex flex-wrap items-end gap-2">
          ...
          <div><label className="block text-xs text-muted-foreground">Category (fund)</label>
            <select value={draft.category} onChange={(e) => setDraft({ ...draft, category: e.target.value })} className="h-9 rounded-md border bg-background px-2 text-sm">{CATEGORIES.map((c) => <option key={c}>{c}</option>)}</select>
          </div>
          ...
          <div><label className="block text-xs text-muted-foreground">Frequency</label>
            <select value={draft.frequency} onChange={(e) => setDraft({ ...draft, frequency: e.target.value })} className="h-9 rounded-md border bg-background px-2 text-sm">{FREQS.map((f) => <option key={f}>{f}</option>)}</select>
          </div>
          ...
          <Card key={g.cat}>
            ...
            <CardContent className="overflow-x-auto p-0">
              <table className="w-full text-sm">
                <thead className="border-b text-left text-xs uppercase text-muted-foreground"><tr><th className="px-3 py-2">Name</th><th className="px-3 py-2">Amount</th><th className="px-3 py-2">Frequency</th><th className="px-3 py-2 text-right">≈ Monthly</th><th className="px-3 py-2">Active</th><th className="px-3 py-2"></th></tr></thead>
                <tbody>
                  {g.items.map((e) => (
                    <tr key={e.id} className={cn("border-b last:border-0", !e.active && "opacity-50")}>
                      ...
                      <td className="px-3 py-2">
                        <select defaultValue={e.frequency} onChange={(ev) => patch(e.id, { frequency: ev.target.value })} className="h-8 rounded-md border bg-background px-1 text-xs">{FREQS.map((f) => <option key={f}>{f}</option>)}</select>
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums text-muted-foreground">{usd(monthly(e.amount, e.frequency))}</td>
