  ...
  try {
    ...
    const products = await conn.execute({
      sql: `SELECT dp.id, dp.brand, dp.title, dp.size, dp.unitMeasure, dp.category, dp.mainImageUrl,
                   dp.bestPrice, dp.bestRetailer, dp.pricePerMeasure, dp.needsReview,
                   json_array_length(dp.imageUrls) AS imgCount,
                   CASE WHEN dp.description IS NOT NULL AND dp.description!='' THEN 1 ELSE 0 END AS hasDesc,
                   CASE WHEN dp.ingredients IS NOT NULL AND dp.ingredients!='' THEN 1 ELSE 0 END AS hasIngr,
                   CASE WHEN dp.nutritionFacts IS NOT NULL AND dp.nutritionFacts!='' THEN 1 ELSE 0 END AS hasNutri,
    ...
    const quality = (await conn.execute(`SELECT
        SUM(CASE WHEN imageUrls IS NOT NULL AND json_array_length(imageUrls)>=5 THEN 1 ELSE 0 END) fullGallery,
        SUM(CASE WHEN description IS NOT NULL AND description!='' THEN 1 ELSE 0 END) withDesc,
        SUM(CASE WHEN bullets IS NOT NULL AND bullets!='[]' AND bullets!='' THEN 1 ELSE 0 END) withBullets,
        SUM(CASE WHEN ingredients IS NOT NULL AND ingredients!='' THEN 1 ELSE 0 END) withIngredients,
        SUM(CASE WHEN nutritionFacts IS NOT NULL AND nutritionFacts!='' THEN 1 ELSE 0 END) withNutrition,
        SUM(CASE WHEN upc IS NOT NULL AND upc!='' THEN 1 ELSE 0 END) withUpc,
        SUM(CASE WHEN needsReview=1 THEN 1 ELSE 0 END) needsReview
      FROM "DonorProduct"`)).rows[0];
