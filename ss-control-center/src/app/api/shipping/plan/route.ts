...
// The rate-choosing rules themselves live in one shared module, so the merged-
// group quote (/api/shipping/merge/rates) runs the identical algorithm instead
// of leaving the operator to eyeball a price list.
import {
  selectBestRate,
  frozenMaxCalDays,
  chooseTodayOrMonday,
...
// (The Veeqo rate shape and the rate-choosing rules now live in
//  @/lib/shipping/rate-selection — imported above.)
        ...
        try {
          ...
          if (tryMonday) {
            try {
              ...
              // The today-vs-Monday comparison itself lives in
              // @/lib/shipping/rate-selection so the merged-group quote makes
              // the same call on the same evidence.
              const { takeMonday, reason: switchReason } = chooseTodayOrMonday({
                todayPick: selectedRate,
                mondayPick,
                todayShipDay: actualShipDay,
