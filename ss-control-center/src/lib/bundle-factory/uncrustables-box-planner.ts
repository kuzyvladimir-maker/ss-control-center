...
export function buildListingCopy(comps: RecipeComponent[]): { title: string; bullets: string[]; description: string } {
  ...
  const bullets = [
    ...
    // БЕЗ дисклеймера сборщика — сознательно. Он куплен инцидентом с
    // ПОДАРОЧНЫМИ НАБОРАМИ под нашим брендом (Salutem Vita), где мы кладём
    // чужой товар в свою корзину и держим Gift Basket Exception. Здесь другое:
    // мультипак товара Smucker's, бренд назван прямо. Замер 2026-08-16:
    // 388 таких листингов приняты Amazon без дисклеймера, 153 из них LIVE,
    // ни одного отказа по 99300. Не чиним то, что не сломано.
