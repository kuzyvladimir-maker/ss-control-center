...
export const validatorBrandField: ValidatorFn = async ({
  ...
  // Walmart lists the ORIGINAL manufacturer brand of the item being resold —
  // owner decision 2026-08-02, standing practice across ~10,000 published
  // listings and several passed account reviews. Reselling genuine goods in
  // their own packaging does not require brand-rights paperwork, so the field
  // simply has to name the true manufacturer. Amazon keeps its allowlist
  // below, where borrowing a foreign brand really is a ranking/IP risk.
  if (sku.channel === "WALMART") {
    return {
      validator_id: "validator-brand-field",
      passed: true,
      details: { brand, source: "walmart_manufacturer_brand" },
    };
  }
