...
function ocrContainsAlias(value: string, aliases: readonly string[]): boolean {
  const tokens = new Set(normalizeVisibleText(value).split(" ").filter(Boolean));
  return aliases.some((alias) => {
    const expectedTokens = normalizeVisibleText(alias).split(" ").filter(Boolean);
    return expectedTokens.length > 0 && expectedTokens.every((token) => tokens.has(token));
  });
}
...
function trustedOcrText(auxiliary: AuditAuxiliaryEvidence | undefined): string {
  if (!auxiliary) return "";
  ...
  return auxiliary.ocr_texts.flatMap((row, index) => {
    ...
    return row.confidence >= WALMART_VISUAL_AUXILIARY_OCR_MIN_CONFIDENCE
      ? [row.text.trim()]
      : [];
  }).join(" ");
}
...
function assessIdentity(
  ...
  const ocrOnlyForbidden = expected.forbidden_markers.filter((marker) =>
    !containsAlias(roleText(observation, marker.role), marker.aliases)
      && ocrContainsAlias(ocrText, marker.aliases));
  ...
  for (const role of roleAssessments) {
    ...
    const supportedGroupMatches = role.groups.map((aliases, index) => {
      const ocrMatch = ocrContainsAlias(ocrText, aliases);
    ...
    const explicitBlindMismatch = hasSpecificText(role.blind) && !blindMatchesAny;
  ...
  if (review.length > 0 || !everyRequiredGroupMatches
...
function isNutrientLiteral(value: string): boolean {
  return /\b(protein|fiber|whole grain|sugar|sugars|sodium|carb|carbs|carbohydrate|fat|calorie|calories|per serving|vitamin|calcium|iron)\b/i.test(value);
}
...
function packageSizesFromBlind(observation: BlindObservation): Record<PackageFactKind, ExpectedSize[]> {
  const visible = dedupeSizes(observation.visible_size_texts
  ...
  const inner = dedupeSizes(observation.inner_contents_claims.flatMap((literal) => {
    ...
    return [...parseVisibleSizeTexts(literal), ...(count ? [count] : [])];
  }));
  return {
    net_content: visible.filter((size) => size.unit !== "count"),
    inner_item_count: dedupeSizes([...visible, ...inner]).filter((size) => size.unit === "count"),
  };
}

function packageSizesFromOcr(ocrText: string): Record<PackageFactKind, ExpectedSize[]> {
  const values = dedupeSizes(parseVisibleSizeTexts(ocrText))
...
function assessPackageFacts(
  ...
  const ocrSizes = packageSizesFromOcr(ocrText);
...
export function auditGallerySlot(input: GalleryAuditInput): GalleryAuditDecision {
  ...
  const packages = composition.kind === "same_product"
    ? assessPackageFacts(input.expected.package_facts, input.source.observation, ocrText)
  ...
  if (input.source.observation.multiple_distinct_products === "yes") {
...
/** Compute orientation-normalized SHA-256 and 64-bit difference hash. */
...
export async function detectGalleryDuplicates(
  ...
  for (const fingerprint of fingerprints) {
    const slots = bySha.get(fingerprint.sha256) ?? [];
    slots.push(fingerprint.slot);
    bySha.set(fingerprint.sha256, slots);
  }
