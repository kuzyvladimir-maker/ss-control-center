...
async function assertPngScreenshot(content: Uint8Array): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      invalid("Screenshot PNG dimensions or page count are outside the evidence contract.");
    }
    // stats() forces libvips to decode pixel data, rejecting truncated/corrupt
    // PNGs that may still expose superficially plausible header metadata.
