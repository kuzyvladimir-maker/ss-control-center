// PREVIEW iteration 5 — rework ONLY listing 5 (#4 passed iteration 4).
// My full check of iteration 4 found on #5:
//   - Blackberry Boom: 3 double-width cartons instead of 4 normal ones (the
//     model fills the row with fewer, wider boxes to span the cooler).
//   - Honey front printed "Peanut Butter & & Honey" (duplicated ampersand
//     across the line break).
// Fix: uniform-dimensions clause for all 4-count cartons + exact front-text
// clause. Texts unchanged from iteration 3.
...
async function main() {
  ...
  for (const r of RECIPES) {
    ...
    mapLines.push(
      ...
      // Iteration-5 additions after my carton-by-carton check of iteration 4:
      "UNIFORM CARTON SIZE: every 4-count carton in the scene has IDENTICAL dimensions — the Blackberry Boom cartons are exactly the same width and height as the Berry Burst cartons. Never draw a wider, double-width or stretched carton; if four equal cartons do not span the cooler, leave the leftover space empty.",
      'EXACT FRONT TEXT: each carton front prints its flavor line exactly once with a single ampersand — the Honey carton reads exactly "Peanut Butter & Honey Spread Sandwich". Never duplicate a word, letter or "&" across a line break.',
    );
  ...
  for (const l of out) {
    ...
    try {
      const t0 = Date.now();
      const res = await generateMainImage({ prompt: l.prompt, r2_path_slug: `preview5-${l.slug}`, reference_urls: l.referenceUrls });
  ...
  writeFileSync("/private/tmp/claude-501/-Users-vladimirkuznetsov-SS-Command-Center/1dbdc77d-9c20-49be-9e0d-c48b604008f6/scratchpad/preview-final-5.json", JSON.stringify(out, null, 1));
