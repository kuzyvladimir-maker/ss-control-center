...
async function main() {
  ...
  for (const r of RECIPES) {
    ...
    mapLines.push(
      ...
      "SIDE BY SIDE ONLY: within a row the cartons stand strictly side by side with small gaps. NEVER place a second carton directly behind a carton of the same flavor — no depth pairs, no double-stacked boxes, no echo edges.",
      "SCENE CONTRACT (mandatory): the scene IS the white Salutem Solutions foam cooler from Reference 1 — open cooler, lid leaning behind, frozen gel packs inside and beside it. The cartons stand in rows inside and above the cooler. This is NEVER a flat catalog lineup of boxes on a plain white background; if the cooler is missing the image is wrong.",
  ...
  writeFileSync("/private/tmp/claude-501/-Users-vladimirkuznetsov-SS-Command-Center/1dbdc77d-9c20-49be-9e0d-c48b604008f6/scratchpad/preview-final-7.json", JSON.stringify(out, null, 1));
