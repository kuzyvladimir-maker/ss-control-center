...
import {
  ...
  renderProductTruthCanonicalCogsReconcileReport,
  type ProductTruthCanonicalCogsReconcilePlan,
  type ProductTruthCanonicalCogsReconcilePreflight,
} from "../src/lib/sourcing/product-truth-canonical-cogs-reconcile";
import {
...
function usage(): string {
  return [
    "Usage:",
    "  node --import tsx scripts/product-truth-canonical-cogs-reconcile.ts plan",
    "    (--url URL | --url-env ENV) [--allow-remote --auth-token-env ENV]",
    "    --manifest-sha256 SHA --listing-key KEY (repeatable, 1-33)",
...
async function runPlan(options: Options): Promise<void> {
  ...
  try {
    const plan = await planProductTruthCanonicalCogsReconciliation({
      db,
    ...
    const planJson = renderProductTruthCanonicalCogsReconcilePlan(plan);
    const planSha256 = sha256(planJson);
    const indexJson = renderProductTruthOperationalJson({
      schemaVersion:
        "product-truth-canonical-cogs-reconcile-artifact-index/1.0.0",
      status: "PLANNED",
...
async function readPlan(options: Options): Promise<{
  ...
  return {
    plan: JSON.parse(planJson) as ProductTruthCanonicalCogsReconcilePlan,
    planJson,
  };
}
...
async function runPreflight(options: Options): Promise<void> {
  ...
  try {
    ...
    const report = await preflightProductTruthCanonicalCogsReconciliation({
      db,
      databaseTargetFingerprint: target.fingerprint,
      plan,
      planJson,
    ...
    const reportJson =
      renderProductTruthCanonicalCogsReconcilePreflight(report);
    const reportSha256 = sha256(reportJson);
    const indexJson = renderProductTruthOperationalJson({
      schemaVersion:
        "product-truth-canonical-cogs-reconcile-preflight-index/1.0.0",
...
async function runApply(options: Options): Promise<void> {
  ...
  try {
    ...
    const report = await applyProductTruthCanonicalCogsReconciliation({
      db,
      databaseTargetFingerprint: target.fingerprint,
      plan,
      planJson,
      ...
      preflight:
        JSON.parse(preflightJson) as ProductTruthCanonicalCogsReconcilePreflight,
      preflightJson,
      preflightSha256: options.preflightSha256!,
      startedAt: options.startedAt!,
    });
    const reportJson = renderProductTruthCanonicalCogsReconcileReport(report);
    const reportSha256 = sha256(reportJson);
    const indexJson = renderProductTruthOperationalJson({
      schemaVersion:
        "product-truth-canonical-cogs-reconcile-result-index/1.0.0",
      status: report.status,
