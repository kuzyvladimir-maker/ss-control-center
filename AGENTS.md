...
### План владельца исполняется целиком, без остановок на отчёт

Одобренный владельцем план или упорядоченный список дефектов — это поручение на
**весь список**, а не меню для согласований. Агент проходит его до конца в одном
заходе и отчитывается один раз, в конце.

Прерываться можно ровно на трёх основаниях:

- работа действительно заблокирована внешним входом;
- нужно решение, которое может принять только владелец;
- следующее действие необратимо и требует его слова.

«Закончил один пункт и написал статус» основанием не является: это стоит
владельцу лишнего круга и читается как просьба о разрешении, которое уже дано.
Длина сессии тоже не основание — если контекст на исходе, об этом говорят одной
строкой и продолжают.

Это не разрешение работать небрежно: каждый пункт по-прежнему проходит
типизацию, линтер, тесты и деплой. Правило про то, **когда говорить**, а не про
то, насколько тщательно делать.
...
- **Норма — не больше 3 зарегистрированных worktree.** Ротация:
  `node scripts/agent-sync.mjs prune --keep=2` (сначала с `--dry-run`).

Ротация не удаляет чекаут, если он грязный, если его коммита нет в основном
репозитории или если в нём была активность за последние 2 часа. Пункт про
коммит критичен: 2026-07-29 внутри «чистых» чекаутов нашлось **9 коммитов,
которых не было больше нигде** — чистка по списку каталогов уничтожила бы их.
...
## 🔐 ЗАПРЕТ: связка ключей macOS (добавлено 29.07.2026 после инцидента)

**НИКОГДА не выполнять `security dump-keychain`** — ни с `-d`, ни без. Этот вызов расшифровывает ВСЮ связку ключей владельца в открытом виде и вызывает каскад системных запросов пароля. 29.07.2026 такой вызов провисел 6 часов, вынудил владельца ввести пароль 5-7 раз и получить право «Разрешать всегда» для `/usr/bin/security`.

Также запрещено без явного разрешения владельца: `security find-generic-password`, `security find-internet-password`, чтение `~/Library/Keychains/*`.

**Как правильно искать секреты:**
1. Переменные окружения текущего процесса.
2. Файлы `.env` / `.env.local` в корне проекта.
3. Если нужного токена нет — **СПРОСИТЬ владельца**, какой токен нужен и куда его положить. Не искать самостоятельно в системных хранилищах.

Причина: связка ключей содержит пароли банков, маркетплейсов и Chrome Safe Storage. Поиск одного API-токена не оправдывает расшифровку всего хранилища.
