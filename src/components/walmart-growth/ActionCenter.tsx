...
/** Pull the headline number out of a "Shipping 15/100" style metric for the big stat. */
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-[13px] text-ink-2">{loading && !data ? "Diagnosing…" : data?.headline ?? "No diagnosis yet."}</div>
        <Btn icon={<RefreshCw size={13} />} onClick={load} loading={loading}>Re-scan</Btn>
      </div>

      {data && data.diagnoses.length === 0 && !loading ? (
        <Panel><div className="p-8 text-center text-[13px] text-ink-3">No major issues detected — or no data yet. Run a Listing Quality sync first.</div></Panel>
      ) : (
            ...
            return (
              <Panel key={dg.id} className="overflow-hidden">
                ...
                <div className="flex items-center gap-3 p-3" style={{ borderLeft: `3px solid ${sev.dot}` }}>
                  <div className="flex h-12 w-14 shrink-0 flex-col items-center justify-center rounded-lg" style={{ background: sev.bg }}>
                    <span className="text-[18px] font-semibold leading-none tabular" style={{ color: sev.color }}>{stat.value}</span>
                    <span className="text-[9px] uppercase tracking-wider" style={{ color: sev.color }}>{stat.sub}</span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="rounded px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider" style={{ background: sev.bg, color: sev.color }}>{sev.label}</span>
                      {dg.itemsAffected != null && <span className="text-[11px] text-ink-3 tabular">{dg.itemsAffected.toLocaleString()} items</span>}
                    </div>
                    <div className="mt-0.5 truncate text-[13px] font-semibold text-ink">{dg.title}</div>
                  </div>
                  <div className="flex shrink-0 items-center gap-1">
                    {dg.action.jumpFilter ? (
                      <Btn size="sm" variant="outline" icon={<ArrowRight size={13} />} onClick={() => onJump(dg.action.jumpFilter!)}>{dg.action.label}</Btn>
                    ) : (
                      <span className={cn("inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium", dg.action.kind === "gated" ? "bg-bg-elev text-ink-3" : "bg-surface-tint text-ink-2 border border-rule")}><Icon size={12} />{dg.action.label}</span>
                    )}
                    <button onClick={() => toggle(dg.id)} className="rounded p-1 text-ink-3 hover:bg-bg-elev hover:text-ink" aria-label="Details">
                ...
                {isOpen && (
                  <div className="border-t border-rule bg-bg-elev/30 px-4 py-3 text-[12px]">
                    ...
                    <p className="mt-1.5 text-ink-3"><span className="font-medium text-ink-2">Why it matters: </span>{dg.why}</p>
                    <p className="mt-1.5"><span className="font-medium text-green-ink">Fix: </span><span className="text-ink-2">{dg.recommendation}</span></p>
