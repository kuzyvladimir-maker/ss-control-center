...
export function buildUncrustablesRetailBoxesContract(input: {
  ...
  withArt.forEach((c) => {
    ...
    mapLines.push(
      `Ref ${refs.length} = the ${c.artSize}-count carton of ${c.flavor}; draw exactly ${boxes} of it, its printed "${c.artSize}" badge and its own fruit artwork unchanged.`,
    );
  });
  ...
  if (input.comps.some((c) => c.flavor === "Whole Wheat Peanut Butter & Strawberry Jam"))
