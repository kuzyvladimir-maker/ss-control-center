...
async function main() {
  ...
  const fails: any[] = JSON.parse(readFileSync("_backlog_final_nodonor.json", "utf8")).filter((x)=>x.title && String(x.title).trim()).filter((x: any) => !OWN.test(x.title));
  const already: any[] = JSON.parse(readFileSync("_backlog_final_fixed.json", "utf8"));
  ...
  const proc = async (members: any[]) => {
    ...
    for (const m of members) {
      ...
      if (donor && pack >= 2) {
        try { const base = await fetchImageBuffer(highResImageUrl(donor.url)); const tile = await uploadToR2(await composeTiledMainImage(base, pack), multipackImageKey(m.sku, "main", "rescue1")); v = await vision.qualifyTiledMain(tile, id?.base_unit || title, pack); if (v.pass) newUrl = tile; } catch (e: any) { v = { reason: "err " + e?.message }; }
      }
